...
export const getAllUsersThunk = createAsyncThunk('auth/getAllUsersThunk', async (obj, thunkAPI) => {
  try {
    const { data } = await baseApi.get(`/users/getAll`);
...
export const getUserById = createAsyncThunk('auth/getUserById', async (obj, thunkAPI) => {
  try {
    const { data } = await baseApi.get(`/users/getById`, obj?.submitData);
