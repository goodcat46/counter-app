...
const MobileFooter = ({ path = '/', ...props }) => {
  const [pageRoutes] = useState([]);
  const filteredNavLinks = pageRoutes.filter(link => link.parentPath === path && link?.navLink !== false);
  const link = pageRoutes.find(link => link.parentPath === path && link?.navLink === false);
      ...
      <div className={[s.MobileFooter, 'theme'].join(' ')}>
        <PrivateComponent>
          ...
          <div className={s.gridCenter}>
            <MaxToTablet>{/* <SlickMobileNavMenu navLinkArr={filteredNavLinks} /> */}</MaxToTablet>
          </div>

          <div className={s.gridRight}>{/* <NavMenu size="40px" /> */}</div>
        </PrivateComponent>
      </div>
    </MaxToTablet>
  );
